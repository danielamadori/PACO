...
def parse(lark_tree, probabilities, impacts, durations, names, delays, loop_probability, loop_round, h = 0, parent = None, index_in_parent = 0, id = 0):
	...
	if lark_tree.data in {'choice', 'natural'}:
		...
		left_child, last_id, left_pending_choice, left_pending_natures = parse(lark_tree.children[0], probabilities, impacts, durations, names, delays, loop_probability, loop_round, id =id + 1, h=h, parent=node, index_in_parent=0)
		right_child, last_id, right_pending_choice, right_pending_natures = parse(lark_tree.children[2], probabilities, impacts, durations, names, delays, loop_probability, loop_round, id =last_id + 1, h=h, parent=node, index_in_parent=1)
		...
		left_child, last_id, left_pending_choice, left_pending_natures = parse(lark_tree.children[0], probabilities, impacts, durations, names, delays, loop_probability, loop_round, id =id + 1, h=h, parent=node, index_in_parent=0)
		right_child, last_id, right_pending_choice, right_pending_natures = parse(lark_tree.children[1], probabilities, impacts, durations, names, delays, loop_probability, loop_round, id =last_id + 1, h=h, parent=node, index_in_parent=1)
